...
import { recordAttempt, saveTestResult } from '../lib/auth'
import './TestAssessment.css'

function TestAssessment({ course, sessionEmail, onClose }) {
  // Record attempt when starting test
  useState(() => {
    recordAttempt(sessionEmail, course.id)
  })
  ...
  const completeSection = (result) => {
    ...
    if (currentSection === sections.length - 1) {
      ...
      saveTestResult(sessionEmail, course.id, finalResults)
